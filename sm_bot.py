...
def banner():
    ...
    print()
    print(r"     sm_bot: github.com/Flazers     ")
    print(RESET_ALL)
...
def main():
    banner()
    print("[1] Start")
    print("[2] Update sm_bot")
    print("[3] Exit")
    print()
    number = input(f"{BRIGHT}{BLUE}Select number: {RESET_ALL}")
    if number == "1":
        spam_handler()
    elif number == "2":
        update()
...
def spam_handler():
    ...
    phone = parse_phone(input(f"{BRIGHT}{BLUE}sm_bot >> {RESET_ALL}"))
    print()
    print("Enter the number of threads")
    threads = input(f"{BRIGHT}{BLUE}sm_bot >> {RESET_ALL}")
    if threads in ["", " ", "0"]:
        threads = "1"
    try:
        if int(threads) > 10:
        ...
        threads = "1"
    banner()
    print(f"Phone: {BRIGHT}{BLUE}{phone}{RESET_ALL}")
    print("sm_bot start", end="\n")
...
def start_spam(phone):
    ...
    while True:
        try:
            ...
            print("{BRIGHT}{GREEN} Sucсess")
        except:
            pass
            print("{BRIGHT}{RED} Fail")
        try:
...
def check_internet():
    try:
        ...
        print(f"\n{BRIGHT}{RED}[*] No internet connection!{RESET_ALL}")
        sleep(1)
        main()
    return
...
def check_version():
    version = "1.2"
    if float(version) < float(get("https://raw.githubusercontent.com/Flazers/sm_bot/master/version.txt").text):
...
def update():
    ...
    update = input(f"{BRIGHT}{BLUE}sm_bot >> {RESET_ALL}")
    if update.lower() == "y":
        if exists("/usr/bin") and isfile("/usr/bin/sm_bot"):
            file = open("/usr/bin/sm_bot", "wb")
        elif exists("/usr/local/bin/") and isfile("/usr/local/bin/sm_bot"):
            file = open("/usr/local/bin/sm_bot", "wb")
        elif exists("/data/data/com.termux/files/usr/bin") and isfile("/data/data/com.termux/files/usr/bin/sm_bot"):
            file = open("/data/data/com.termux/files/usr/bin/sm_bot", "wb")
        try:
            file.write(get("https://raw.githubusercontent.com/Flazers/sm_bot/master/sm_bot.py").content)
            file.close()
            system("sm_bot")
        except UnboundLocalError:
            system("cd $HOME && rm -rf sm_bot && git clone https://github.com/Flazers/sm_bot/ && cd sm_bot && sh install.sh")
    else:
        main()
